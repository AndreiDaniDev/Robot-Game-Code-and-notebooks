#All of our programs are made in Spike Prime 3.4.0
import runloop, hub, motor, motor_pair, color

async def gyroForwardNoShow(distanta: float, viteza: int, target: int = 0, * , kp: float = 0.25, kd: float = 0.3, stop: bool = True, resetGyro: bool = True):
...
async def gyroForwardShow(distanta: float, viteza: int, target: int = 0, * , kp: float = 0.25, kd: float = 0.3, ki: float = 0, iterator : int = 5, stop: bool = True, resetGyro: bool = True):
    d = int(distanta * 20.45); lastError = 0
    if(resetGyro): hub.motion_sensor.reset_yaw(0);
    motor.reset_relative_position(hub.port.B, 0)
    integral = 0; i = 0;
...
async def gyroBackwardsShow(distanta: float, viteza: int, target: int = 0, * , kp: float = 0.25, kd: float = 0.3, ki: float = 0, iterator : int = 5, stop: bool = True, resetGyro: bool = True):
    d = int(distanta * 20.45); lastError = 0; viteza *= -1
    if(resetGyro): hub.motion_sensor.reset_yaw(0);
    motor.reset_relative_position(hub.port.B, 0)
...
async def turnLeftShow(grade: float, vitezaMax: int = 500, vitezaMin: int = 150, * , resetGyro: bool = True, kp: float = 0.25, kd: float = 0.35, iterator : int = 5):
    if(resetGyro): hub.motion_sensor.reset_yaw(0)
    target = int(grade * 10)
    eroare = float(target - abs(hub.motion_sensor.tilt_angles()[0]))
...
async def turnRightShow(grade: float, vitezaMax: int = 500, vitezaMin: int = 150, * , resetGyro: bool = True, kp: float = 0.25, kd: float = 0.35, iterator : int = 5):
    if(resetGyro): hub.motion_sensor.reset_yaw(0)
    target = int(grade * 10)
    eroare = float(target - abs(hub.motion_sensor.tilt_angles()[0]))
    lastError = target; i = 0
